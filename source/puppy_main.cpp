...
#include "puppy_primitive.hpp"
#include "puppy_primitiveinline.hpp"
#include "puppy_randomizer.hpp"
    ...
    unsigned int inGeneration, QString& ioOS, double& best_fit, int& best_index, double& avgSize, double& maxSize, double& minSize)
{
    ...
    ioOS = "Gen: " + QString::number(inGeneration) + ", avg: " + QString::number(lAvg) + ", std: " + QString::number(lStd) + ", max: " + QString::number(lMax) + ", min: " + QString::number(lMin) + ", avg Size: " + QString::number(avgSize);
    best_fit = lMin;
    maxSize = sMax;
    minSize = sMin;
}
    ...
    unsigned int inMaxTreeDepth)
{
    ...
    for (unsigned int i = 0; i < 7; ++i) {
        ...
        unsigned int lNewDepthTree1 = lStack1.size() + ioTree2.getDepth(lChoosenNode2) - 1;
        ...
        unsigned int lNewDepthTree2 = lStack2.size() + ioTree1.getDepth(lChoosenNode1) - 1;
    ...
    unsigned int inDepth)
{
    ...
    if (inDepth == 1) {
        ...
        PrimitiveHandle lTerminal = ioContext.mTerminalSet[ioContext.mRandom(ioContext.mTerminalSet.size())];
        ioTree.push_back(Node(lTerminal->giveReference(ioContext), 1));
        return 1;
    }

    assert(ioContext.mFunctionSet.size() > 0);
    PrimitiveHandle lFunction = ioContext.mFunctionSet[ioContext.mRandom(ioContext.mFunctionSet.size())];
    ...
    unsigned int inMaxDepth)
{
    ...
    if (inMinDepth > 1) {
        ...
        unsigned int lIndexSel = ioContext.mRandom(ioContext.mFunctionSet.size() + ioContext.mTerminalSet.size());
    ...
    unsigned int inMaxDepth)
{
    ...
    lNewTree.insert(lNewTree.end(),
        ...
        ioTree.end());
    unsigned int lDiffSize = ioTree[lMutIndex].mSubTreeSize - lNewTree[lMutIndex].mSubTreeSize;
    ...
    float inDistribProba)
{
    ...
    if (lNbArgs == 0) {
        ...
        PrimitiveHandle lFunction = ioContext.mFunctionSet[lKArgsFunction[ioContext.mRandom(lKArgsFunction.size())]];
    ...
    Puppy::Context& ioContext)
{
    ...
    for (unsigned int i = 0; i < ioPopulation.size(); ++i) {
        ...
        while (lDice < lRoulette[lChoosen])
            ++lChoosen;
        ++lIndices[lChoosen];
    }
